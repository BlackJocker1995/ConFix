...
class FixMavlinkAPM(DroneMavlink):
    """
    ...
    def __init__(self, port, recv_msg_queue, send_msg_queue):
        super(FixMavlinkAPM, self).__init__(port, recv_msg_queue, send_msg_queue)
    ...
    def extract_log_file(log_file):
        ...
        while True:
            ...
            if msg.get_type() in ['ATT', 'RATE']:
                out_data.append(FixMavlinkAPM.log_extract_apm(msg))
        ...
        pd_array = FixMavlinkAPM.fill_and_process_pd_log(pd_array)
        return pd_array
    ...
    def extract_log_path(log_path, skip=True, threat=None):
        ...
        if threat is not None:
            ...
            for array in arrays:
                threat_manage.append(FixMavlinkAPM.extract_log_path_threat.remote(log_path, array, skip))
            ...
            for file in tqdm(file_list):
                ...
                # extract
                # try:
                csv_data = FixMavlinkAPM.extract_log_file(log_path + f'/{file}')
    ...
    def extract_from_ulog(log_file):
        ...
        for i in range(len(label) - 2):
            if label[i:i + 3].sum() == 3:
                out = FixMavlinkAPM.log_extract_px4(array.iloc[i:i + 3])
                data.append(out)
...
class FlyFixMavlinkAPM(DroneMavlink):
    ...
    def init_current_param(self):
        ...
        while len(self.param_current) <= len(accpet_param):
            ...
            if msg.Name in accpet_param:
                self.param_current.update(FixMavlinkAPM.log_extract_apm(msg))
    ...
    def read_status_patch_bin(self, time_last, time_unit: float):
        ...
        first_msg = FixMavlinkAPM.log_extract_apm(first_msg)
        ...
        while new_time < first_time + time_unit + 0.1:
            ...
            if msg.get_type() in ['ATT', 'RATE']:
                ...
                tmp = FixMavlinkAPM.log_extract_apm(msg)
                tmp.pop("TimeS")
...
class FixMavlinkPX4(DroneMavlink):
    """
    Mainly responsible for initiating the communication link to interact with UAV
    """

    def __init__(self, port, recv_msg_queue, send_msg_queue):
        super(FixMavlinkPX4, self).__init__(port, recv_msg_queue, send_msg_queue)
