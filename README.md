# Nyctea
A patch repair approach to solve range specification bugs in drone control program.
...
* PARAM_PART: the parameter that participate in repair.
...
`3.feature_split.py` split the csv data for train and test.

`3.train_predictor_tcn.py` train a model predictor.

`4.test_tcn.py` start the predictor test.

`5.fly_and_correct*.py` start a mission with Nyctea monitor.

`5.fly_and_correct_test.py` a repair example for a configuration.
